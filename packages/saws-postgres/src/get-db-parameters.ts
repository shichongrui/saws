...
export const getDBPasswordParameterName = (name: string, stage: string) => `${stage}-${name}-postgress-user-password`
...
export const getDBPassword = async (
  ...
  try {
    const password = await secretsManager.get(getDBPasswordParameterName(name, stage));
    return password;
  } catch (err) {
    if ((err as Error).name !== "ParameterNotFound") throw err;

    const newPassword = await generateToken();
    await secretsManager.set(getDBPasswordParameterName(name, stage), newPassword);
    return newPassword;
  }
};

export const getDBUsername = (name: string, stage: string = process.env.STAGE as string) => {
  return `${name.replace(/[^a-zA-Z\d]/g, "_")}_${stage}`;
};

export const getDBName = (name: string, stage: string = process.env.STAGE as string) => {
  return `${name.replace(/[^a-zA-Z\d]/g, "_")}_${stage}`;
};
