...
test("defines a redis-cli ping health check that can be overridden", async () => {
  ...
  assert.deepEqual(
    (await service.getRunConfig(new DevContext({ stage: "dev" })))
      .healthCheck,
    {
  ...
  assert.equal(
    (await disabled.getRunConfig(new DevContext({ stage: "dev" })))
      .healthCheck,
    false
  );
});
...
test("runs redis-server with requirepass and persistence", async () => {
  ...
  const config = await service.getRunConfig(
    new DevContext({ stage: "dev" })
  );
