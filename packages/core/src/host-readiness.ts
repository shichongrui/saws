...
export function readinessCheckScript(config: HostReadinessConfig) {
...
if [ "$actual" != "$expected" ]; then
  echo ${shellQuote(
    `Host "${config.name}" is not configured for this deployment. Run: ${configureCommand}`,
  )} >&2
  exit 78
fi
...
if [ -z "$deployment_home" ] ||
  ...
  echo ${shellQuote(
    `Deployment SSH access has drifted on host "${config.name}". Run: ${configureCommand}`,
  )} >&2
  exit 78
fi
...
export function readinessConfigureScript(config: HostReadinessConfig) {
