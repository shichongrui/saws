...
export class Api implements ModuleDefinition, ApiConfig {
      ...
      const server = http.createServer(
        async (req: IncomingMessage, res: ServerResponse) => {
          ...
          try {
            ...
            res.writeHead(results.statusCode, {...results.headers, ...results.multiValueHeaders });
  ...
  async deploy(stage: string) {
    ...
    const permissions = this.dependencies.map((dependency) => dependency.getPermissions(this.type, process.env.STAGE!)).flat()
