...
import {
  Postgres,
  postgresServiceUrlEnvironmentVariable,
  resolvePostgresServiceUrl,
} from "./index.js";

test("derives the database URL variable used by a Postgres service", () => {
  assert.equal(
    postgresServiceUrlEnvironmentVariable("primary-db"),
...
test("resolves a service URL from an application environment", () => {
  assert.equal(
    resolvePostgresServiceUrl("primary-db", {
      PRIMARY_DB_DATABASE_URL:
...
test("reports the expected variable when configuration is missing", () => {
  assert.throws(
    () => resolvePostgresServiceUrl("missing-db", {}),
    /MISSING_DB_DATABASE_URL/
  );
});
