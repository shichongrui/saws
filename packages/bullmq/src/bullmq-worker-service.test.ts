...
class FakeRedisService extends ServiceDefinition {
  ...
  constructor(
    opts: { name?: string; host?: string; port?: string; password?: string } = {},
  ) {
  ...
  override async getEnvironmentVariables(
    ...
    const host =
      target === "container" ? `${context.stage}-${this.name}` : this.redisHost;
...
class TestBullMQWorkerService extends BullMQWorkerService {
  ...
  getRunConfig(context: RuntimeContext, provider = this.providers[0]) {
...
function createRegistryProvider(name: string, registry: string) {
  return new RecordingDockerProvider({
...
test("supports a custom and an empty queue name prefix", async () => {
  ...
  assert.equal(
    (await custom.getEnvironmentVariables(context, "host"))["W_QUEUE_NAME"],
    "custom-emails",
  );
  ...
  assert.equal(
    (await none.getEnvironmentVariables(context, "host"))["W_QUEUE_NAME"],
    "emails",
  );
});
...
test("deploys the worker on every provided host", async () => {
  ...
  assert.equal(
    providerA.remoteRuns[0]?.image,
    "registry.a/team/production-email-worker:latest",
  );
  assert.equal(
    providerB.remoteRuns[0]?.image,
  ...
  assert.deepEqual(
    providerA.builds.map((build) => build.image),
    ["registry.a/team/production-email-worker:latest"],
  );
