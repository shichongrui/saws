...
`PostgresDockerService` applies pending SQL migrations from
`<service-name>/migrations` with dbmate after its PostgreSQL container starts
during `saws deploy`.
...
The command sets dbmate's migrations directory to
`<service-name>/migrations`. When multiple Postgres services are configured,
select one with `--service <service-name>`.
