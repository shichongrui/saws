...
test("initializes dbmate and the migrations directory without replacing package metadata", async () => {
  ...
  assert.equal(
    (await stat(path.join(rootDir, "primary-db", "migrations"))).isDirectory(),
    true
  );
...
test("applies dbmate migrations on the deployment Docker network", async () => {
  ...
  assert.equal(
    host.copies.some(({ remotePath, contents }) =>
      remotePath === "/srv/saws/production/primary-db/migrations.env" &&
      contents ===
  ...
  assert.match(
    migrationRun ?? "",
    /-v '\/srv\/saws\/production\/primary-db\/migrations:\/db\/migrations:ro'/
  );
  assert.match(
    migrationRun ?? "",
