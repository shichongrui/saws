...
import { createMigrateCommand, migrateCommand, } from "./migrate-command.js";
test("runs a dbmate command with the project-local executable", async () => {
    const calls = [];
    await migrateCommand(["up"], { rootDir: "project" }, async (command, args, options) => {
...
test("uses the configured service migrations directory", async () => {
    ...
    const command = createMigrateCommand(async (_executable, _args, options) => {
        migrationsDirectory = options.env.DBMATE_MIGRATIONS_DIR;
    }, ["primary-db"]);
    await command.parseAsync(["node", "saws", "up"]);
    assert.equal(migrationsDirectory, path.resolve("primary-db", "migrations"));
});
test("selects a migrations directory when multiple Postgres services exist", async () => {
    ...
    assert.equal(migrationsDirectory, path.resolve("analytics", "migrations"));
});
