...
export class PostgresDockerService extends DockerService {
    static getCommands(services = []) {
        return [
            createMigrateCommand(undefined, services.map((service) => service.name)),
        ];
    }
    port;
    database;
    ...
    async onInit(context) {
        ...
        const migrationsDirectory = path.join(context.rootDir, this.name, "migrations");
        if (context.dryRun) {
        ...
        if (packageChanged ||
            !(await fileExists(path.join(context.rootDir, "node_modules", ".bin", "dbmate")))) {
    ...
    async runMigrations(context) {
        const sourceDirectory = path.join(context.rootDir, this.name, "migrations");
        const migrationFiles = await listMigrationFiles(sourceDirectory);
        const relativeDirectory = `${this.name}/migrations`;
        const remoteDirectory = path.posix.join(this.docker.getAppDirectory(context), relativeDirectory);
        const runtimeFiles = [];
        ...
        try {
            for (const fileName of migrationFiles) {
                runtimeFiles.push(await this.docker.writeRuntimeFile(context, `${relativeDirectory}/${fileName}`, await readFile(path.join(sourceDirectory, fileName), "utf8")));
            }
            ...
            const environmentFile = await this.docker.writeRuntimeFile(context, `${this.name}/migrations.env`, `DATABASE_URL=${databaseUrl}\n`);
            runtimeFiles.push(environmentFile);
            await this.docker.host.exec(`docker pull ${this.docker.host.shellQuote(this.migrationImage)}`, { dryRun: context.dryRun });
            await this.docker.host.exec([
                "docker run --rm",
            ...
            ].join(" "), { dryRun: context.dryRun });
        }
        finally {
            for (const runtimeFile of runtimeFiles.reverse()) {
                await this.docker.removeRuntimeFile(context, runtimeFile);
            }
            await this.docker.host.exec(`rm -rf ${this.docker.host.shellQuote(remoteDirectory)}`, { dryRun: context.dryRun });
        }
    }
...
async function ensureDbmateDependency(context) {
...
function withSslModeDisabled(databaseUrl) {
