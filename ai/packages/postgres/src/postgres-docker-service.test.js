...
test("initializes dbmate and the migrations directory without replacing package metadata", async () => {
    ...
    assert.equal((await stat(path.join(rootDir, "primary-db", "migrations"))).isDirectory(), true);
...
test("reports the published host port while container connections use port 5432", async () => {
    ...
    assert.deepEqual(await service.getEnvironmentVariables(context, "container"), {
        ...
        DB_POSTGRES_PASSWORD: "secret",
        DB_POSTGRES_DB_NAME: "app",
        DB_DATABASE_URL: "postgresql://app-user:secret@development-db:5432/app",
    });
...
test("creates the configured database and user with a secret-backed password", async () => {
    ...
    assert.equal((await service.getEnvironmentVariables(context)).DB_DATABASE_URL, "postgresql://application-user:secret%20from%20manager@localhost:5432/application");
});
test("defines a pg_isready health check that can be overridden", async () => {
    ...
    assert.deepEqual((await service.getRunConfig(new DevContext({ stage: "dev" }))).healthCheck, {
    ...
    assert.equal((await disabled.getRunConfig(new DevContext({ stage: "dev" }))).healthCheck, false);
});
test("applies dbmate migrations on the deployment Docker network", async () => {
    ...
    const migrationsDirectory = path.join(rootDir, "primary-db", "migrations");
    await mkdir(migrationsDirectory, { recursive: true });
    await writeFile(path.join(migrationsDirectory, "20260703120000_create_users.sql"), "-- migrate:up\ncreate table users (id bigint primary key);\n\n-- migrate:down\ndrop table users;\n");
    await writeFile(path.join(migrationsDirectory, "README.md"), "Only SQL migration files should be uploaded.\n");
    ...
    await service.deploy(new DeployContext({ stage: "production", rootDir }));
    assert.equal(host.copies.some(({ remotePath, contents }) => remotePath ===
        "/srv/saws/production/primary-db/migrations/20260703120000_create_users.sql" &&
        contents.includes("create table users")), true);
    assert.equal(host.copies.some(({ remotePath }) => remotePath.endsWith("README.md")), false);
    assert.equal(host.copies.some(({ remotePath, contents }) => remotePath === "/srv/saws/production/primary-db/migrations.env" &&
        contents ===
            "DATABASE_URL=postgresql://app-user:secret@production-primary-db:5432/app?sslmode=disable\n"), true);
    const migrationRun = host.commands.find((command) => command.startsWith("docker run --rm"));
    assert.match(migrationRun ?? "", /--network 'saws-production'/);
    assert.match(migrationRun ?? "", /-v '\/srv\/saws\/production\/primary-db\/migrations:\/db\/migrations:ro'/);
    assert.match(migrationRun ?? "", /'ghcr\.io\/amacneil\/dbmate:v2\.33\.0' --wait --no-dump-schema migrate$/);
    assert.ok(host.commands.findIndex((command) => command.startsWith("docker run --rm")) >
...
test("still runs dbmate when the migrations directory is absent", async () => {
    ...
    await service.deploy(new DeployContext({ stage: "production", rootDir }));
    assert.match(host.commands.find((command) => command.startsWith("docker run --rm")) ?? "", /'registry\.example\.test\/dbmate:pinned' --wait --no-dump-schema migrate$/);
});
test("fails deployment and removes temporary files when migrations fail", async () => {
    ...
    await assert.rejects(service.deploy(new DeployContext({ stage: "production", rootDir })), /migration failed/);
    assert.equal(host.commands.some((command) => command === "rm -f '/opt/saws/production/db/migrations.env'"), true);
    assert.equal(host.commands.at(-1), "rm -rf '/opt/saws/production/db/migrations'");
});
