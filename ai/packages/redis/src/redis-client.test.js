...
import { Redis, redisServiceUrlEnvironmentVariable, resolveRedisServiceUrl, } from "./redis-client.js";
test("derives the Redis URL variable used by a Redis service", () => {
    assert.equal(redisServiceUrlEnvironmentVariable("session-cache"), "SESSION_CACHE_REDIS_URL");
});
test("resolves a service URL from an application environment", () => {
    assert.equal(resolveRedisServiceUrl("session-cache", {
        SESSION_CACHE_REDIS_URL: "redis://:secret@localhost:6379",
...
test("reports the expected variable when configuration is missing", () => {
    assert.throws(() => resolveRedisServiceUrl("missing-redis", {}), /MISSING_REDIS_REDIS_URL/);
});
