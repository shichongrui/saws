...
import { ParameterNotFoundError, SecretsManager, } from "./secrets-manager.js";
test("resolves a secret reference against the runtime stage and root", async () => {
    ...
    await writeFile(path.join(rootDir, ".env"), `SAWS_SECRETS_PASSCODE=${passcode}\n`);
    ...
    assert.equal(await reference.resolve({ stage: "production", rootDir }), "production-token");
    assert.equal(await reference.resolve({ stage: "staging", rootDir }), "staging-token");
});
test("rejects empty references and missing referenced secrets", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "saws-secrets-"));
    assert.throws(() => SecretsManager.reference("  "), /name cannot be empty/);
    await assert.rejects(SecretsManager.reference("missing").resolve({
        stage: "production",
        rootDir,
...
test("encrypts secrets on disk and generates a passcode in .env", async (t) => {
    ...
    t.after(() => {
        if (inheritedPasscode == null)
            delete process.env.SAWS_SECRETS_PASSCODE;
        else
            process.env.SAWS_SECRETS_PASSCODE = inheritedPasscode;
    });
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "saws-secrets-"));
...
test("rejects an incorrect passcode", async () => {
    ...
    await assert.rejects(new SecretsManager({
        stage: "production",
        rootDir,
        passcode: "incorrect",
    }).get("api-key"), /Unable to decrypt secrets/);
});
