...
export type DockerHost = Pick<
  Host,
  "name" | "address" | "exec" | "copyFile" | "shellQuote" | "assertReady"
>;

interface DockerProviderBaseConfig {
  host: DockerHost;
  appDirectory?: string;
  network?: string;
}
...
export interface DockerRegistryAuthConfig {
  username: string;
  /** Stage-aware secret containing the registry password or access token. */
  password: SecretReference;
}

export type DockerProviderConfig = DockerProviderBaseConfig & (
  | {
      registry?: undefined;
      auth?: never;
    }
  | {
      /** Registry (and optional namespace) used for images built during deploy. */
      registry: string;
...
export interface DockerRunConfig {
  ...
  /** Pull the image before running it. Defaults to true. */
  pull?: boolean;
  /** Precomputed before temporary env files replace inline environment values. */
  configHash?: string;
}
...
export interface DockerBlueGreenConfig {
  /** App container configuration. Its name is used as the slot-name prefix. */
  app: DockerRunConfig;
  /** Port exposed by the app inside the Docker network. */
  appPort: number;
  /** Host-to-proxy port mappings. */
  proxyPorts: string[];
  /** HTTP path polled before traffic is switched. Defaults to "/". */
  healthCheckPath?: string;
  /** Seconds to wait for app readiness. Defaults to 30. */
  healthCheckTimeoutSeconds?: number;
  /** Seconds old Nginx workers and the previous app may drain. Defaults to 30. */
  drainTimeoutSeconds?: number;
  /** Nginx image used by the stable proxy container. */
  proxyImage?: string;
}
...
export class DockerProvider {
  ...
  private readonly locallyAuthenticatedContexts = new WeakSet<DeployContext>();
  private readonly remotelyAuthenticatedContexts = new WeakSet<DeployContext>();
  ...
  constructor(config: DockerProviderConfig) {
    ...
    if (this.auth != null) {
      ...
      if (!(this.auth.password instanceof SecretReference)) {
        throw new Error(
          "DockerProvider registry auth password must be a SecretsManager reference"
        );
      }
    }
  }

  getNetwork(context: RuntimeContext) {
    return `${this.network}-${context.stage}`;
  }

  getAppDirectory(context: RuntimeContext) {
    return path.posix.join(this.appDirectory, context.stage);
  }

  getBuiltImageName(context: RuntimeContext, serviceName: string) {
    ...
    if (context instanceof DeployContext) {
      if (this.registry == null || this.registry.length === 0) {
        throw new Error(
          `Docker service "${serviceName}" uses a Dockerfile, but its DockerProvider has no registry configured`
        );
      }
  ...
  async buildImage(context: RuntimeContext, config: DockerBuildConfig) {
    const dockerfile = path.resolve(context.rootDir, config.dockerfile);
    const buildContext = path.resolve(
      context.rootDir,
      config.context ?? path.dirname(config.dockerfile)
    );
    await runLocal(
      [
        "docker build",
      ...
      { dryRun: context.dryRun, context }
    );
  }

  async pushImage(context: DeployContext, image: string) {
  ...
  async prepare(context: DeployContext, network = this.getNetwork(context)) {
    await this.authenticateRemoteRegistry(context);
    await this.host.exec(
      `mkdir -p ${this.host.shellQuote(this.getAppDirectory(context))}`,
      { dryRun: context.dryRun }
    );
    await this.host.exec(
      `docker network inspect ${this.host.shellQuote(network)} >/dev/null 2>&1 || docker network create ${this.host.shellQuote(network)}`,
      { dryRun: context.dryRun }
    );
  }

  async assertHostReady(context: DeployContext) {
    await this.host.assertReady({ dryRun: context.dryRun });
  }

  private async authenticateLocalRegistry(context: DeployContext) {
    if (
      this.registry == null ||
      this.auth == null ||
  ...
  private async authenticateRemoteRegistry(context: DeployContext) {
    if (
      this.registry == null ||
      this.auth == null ||
  ...
  async runContainer(context: DeployContext, config: DockerRunConfig) {
    ...
    if (config.pull !== false) {
      await this.host.exec(
        `docker pull ${this.host.shellQuote(config.image)}`,
        { dryRun: context.dryRun }
      );
    }
    ...
    const runCommand = [
      ...
      .join(" ");
    const currentHash =
      `$(docker inspect --format ${this.host.shellQuote('{{index .Config.Labels "saws.configHash"}}')} ${containerName} 2>/dev/null || true)`;
    const currentImage =
      `$(docker inspect --format ${this.host.shellQuote("{{.Image}}")} ${containerName} 2>/dev/null || true)`;
    const desiredImage =
      `$(docker image inspect --format ${this.host.shellQuote("{{.Id}}")} ${image})`;
    const isRunning =
      `$(docker inspect --format ${this.host.shellQuote("{{.State.Running}}")} ${containerName} 2>/dev/null || true)`;
  ...
  async runBlueGreenContainer(
    ...
    const serviceName =
      config.app.labels?.["saws.service"] ?? config.app.name;
    const proxyDirectory = path.posix.join(
      this.getAppDirectory(context),
      serviceName,
      "proxy"
    );
    ...
    const appConfigHash =
      config.app.configHash ?? this.getContainerConfigHash({
        ...config.app,
        network,
      });
    ...
    await this.host.exec(
      `mkdir -p ${this.host.shellQuote(proxyDirectory)}`,
      { dryRun: context.dryRun }
    );
    if (config.app.pull !== false) {
      await this.host.exec(
        `docker pull ${this.host.shellQuote(config.app.image)}`,
        { dryRun: context.dryRun }
      );
    }
    await this.host.exec(
      `docker pull ${this.host.shellQuote(proxyImage)}`,
      { dryRun: context.dryRun }
    );

    const appRunCommand = (slotName: string) =>
    ...
    const proxyHealthCheckArgs = this.getDockerHealthCheckArgs({
      command: [
        ...
        quotePosixShell(
          `http://127.0.0.1:${config.appPort}${healthCheckPath}`
        ),
      ].join(" "),
      interval: "10s",
      timeout: "5s",
  ...
  getContainerConfigHash(config: DockerRunConfig) {
  ...
  async prepareLocal(
    context: DevContext,
    network = this.getNetwork(context)
  ) {
    await runLocal(
      `docker network inspect ${this.host.shellQuote(network)} >/dev/null 2>&1 || docker network create ${this.host.shellQuote(network)}`,
      { dryRun: context.dryRun, context }
  ...
  async runLocalContainer(context: DevContext, config: DockerRunConfig) {
    const network = config.network ?? this.getNetwork(context);
    ...
    await runLocal(
      `docker rm -f ${this.host.shellQuote(config.name)} >/dev/null 2>&1 || true`,
      { dryRun: context.dryRun, context }
    );
    await runLocal(
      [
  ...
  async startLocalContainer(
    ...
    if (context.dryRun) {
      context.writeLog(
        `[dry-run:local] docker ${args.map((arg) => this.host.shellQuote(arg)).join(" ")}\n`
      );
  ...
  async writeRuntimeFile(
    ...
    await this.host.exec(
      `mkdir -p ${this.host.shellQuote(path.posix.dirname(remotePath))}`,
      { dryRun: context.dryRun }
    );
  ...
  async removeRuntimeFile(context: DeployContext, runtimeFile: RuntimeFile) {
    await rm(runtimeFile.localPath, { force: true });
    await this.host.exec(
      `rm -f ${this.host.shellQuote(runtimeFile.remotePath)}`,
      { dryRun: context.dryRun }
    );
  }

  async writeLocalRuntimeFile(
    context: DevContext,
    relativePath: string,
    contents: string
  ): Promise<string> {
  ...
  async removeLocalRuntimeFile(localPath: string) {
    await rm(localPath, { force: true });
  }

  private getDockerRunCommand(config: DockerRunConfig) {
    const envArgs = Object.entries(config.env ?? {})
      .map(([key, value]) => `-e ${this.host.shellQuote(`${key}=${value}`)}`)
      .join(" ");
    const envFileArgs = (config.envFiles ?? [])
  ...
  private getDockerHealthCheckArgs(
    ...
    | { flag: string; flagOnly: true }
    | { flag: string; flagOnly: false; value: string }
  > {
