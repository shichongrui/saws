...
function createProvider(registry) {
...
test("requires exactly one Docker image source", () => {
    ...
    assert.throws(() => new DockerService({ name: "web", docker }), /requires exactly one of image or dockerfile/);
    assert.throws(() => new DockerService({
        name: "web",
        docker,
        image: "nginx:alpine",
...
test("rejects stages that are unsafe as Docker names or path segments", () => {
    assert.throws(() => new DeployContext({ stage: "../production" }), /Invalid stage/);
    assert.throws(() => new DeployContext({ stage: "Production" }), /Invalid stage/);
});
...
test("renders and hashes Docker health check configuration", async () => {
    ...
    await docker.runContainer(new DeployContext({ stage: "production" }), withHealthCheck);
    ...
    assert.notEqual(docker.getContainerConfigHash(base), docker.getContainerConfigHash(withHealthCheck));
});
test("can disable a health check inherited from an image", async () => {
    ...
    await docker.startLocalContainer(new DevContext({
        stage: "dev",
        dryRun: true,
...
test("exits before image or container work when the host is not ready", async () => {
    ...
    await assert.rejects(service.deploy(new DeployContext({ stage: "production" })), /saws host configure test/);
    assert.equal(docker.builds.length, 0);
    assert.equal(docker.pushes.length, 0);
...
test("requires a registry when deploying a Dockerfile service", async () => {
    ...
    await assert.rejects(service.deploy(new DeployContext({ stage: "production" })), /has no registry configured/);
});
test("requires secret-backed auth with a configured registry", () => {
    const host = new Host({ name: "test", address: "example.test" });
    assert.throws(() => new DockerProvider({
        host,
        registry: "///",
        auth: {
            username: "test-user",
...
test("logs in locally and remotely once per deployment context", async () => {
    ...
    assert.equal(localLogs.match(/docker login/g)?.length, 1, "local login should run once");
    assert.match(localLogs, /docker login 'registry\.example\.com' --username 'deploy-user' --password-stdin/);
    assert.doesNotMatch(localLogs, /super-secret-token/);
    const remoteLogins = host.calls.filter(({ command }) => command.startsWith("docker login"));
...
test("resolves Dockerfile and default build context paths from the runtime root", async () => {
    ...
    await docker.buildImage(new DevContext({
        stage: "dev",
        rootDir: "/project",
        dryRun: true,
        logSink: ({ chunk }) => logs.push(chunk),
...
test("isolates generated runtime files by stage locally and remotely", async () => {
    ...
    const remote = await docker.writeRuntimeFile(staging, "api/container.env", "TOKEN=staging\n");
    const local = await docker.writeLocalRuntimeFile(production, "api/container.env", "TOKEN=production\n");
    assert.equal(remote.localPath, path.join(rootDir, ".saws", "hosts", "main", "staging", "api", "container.env"));
    assert.equal(remote.remotePath, "/srv/my-app/staging/api/container.env");
    assert.equal(host.copies[0]?.remotePath, remote.remotePath);
    assert.equal(local, path.join(rootDir, ".saws", "local", "production", "api", "container.env"));
...
test("runs blue/green app slots behind a stable Nginx proxy", async () => {
    ...
    await docker.runBlueGreenContainer(new DeployContext({ stage: "production" }), config);
    ...
    assert.match(rollout, /\/opt\/saws\/production\/api\/proxy\/nginx\.conf/);
    ...
    assert.equal(spawnSync("/bin/sh", ["-n"], { input: rollout }).status, 0, "generated rollout command must be valid POSIX shell");
});
